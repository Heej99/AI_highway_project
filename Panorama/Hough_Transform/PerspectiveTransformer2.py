    ...
    def makeImagesList(self):       
        ...
        while cap.isOpened():
            ...
            if ret:
                ...
                ## find perspective lane
                l_detect = LaneDetect.L_Detect(frame)
                left,right = l_detect.Hough_transform()
                ...
                i+=1
                if i==500: # 1000개 이미지 뽑으면 그만 하기 <- 임의로 지정
                    break
                
                ## save image
                cv2.imwrite('./data/FHD/' + 'fhd'+ '_' + str(currentFrameNumber) + '.jpg', frame)
                cv2.imwrite('./data/transformedFHD/' + 'tfhd'+ '_' + str(currentFrameNumber) + '.jpg', curr_cropimg)
            #print(result)
